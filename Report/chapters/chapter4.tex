...
% TODO: Required Asset Name: AI Dataset Sample
% Asset Type: Missing Technical Visual Asset
% Status: Pending Asset Collection
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/chapter4/ai_dataset_sample.png}
    \caption{[Placeholder] AI Dataset Sample}
    \label{fig:placeholder_ai_dataset_sample}
\end{figure}
...
% TODO: Required Asset Name: AI Training Metrics
% Asset Type: Missing Technical Visual Asset
% Status: Pending Asset Collection
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/chapter4/ai_training_metrics.png}
    \caption{[Placeholder] AI Training Metrics}
    \label{fig:placeholder_ai_training_metrics}
\end{figure}
...
application (Android/Flutter), and the AI/ML recommendation service 
\cite{thilakarathne2023fields}. The implementation is aligned with 
the system design described in Chapter~\ref{ch:system-design}.
...
\begin{itemize}[leftmargin=1.2cm]
    \item Consistent naming conventions were used across all modules 
    (camelCase for variables/functions, PascalCase for classes).
    \item Functions were kept small and single-purpose to improve 
    modularity and testability \cite{sommerville2016software}.
    \item All modules include meaningful comments for complex logic 
    and configuration parameters.
    \item Sensitive information (API keys, tokens) was kept out of 
    source code and stored using environment variables or secure 
    configuration files \cite{jwtauth2017}.
    \item Proper error handling and validation were applied at the 
    boundaries (API inputs, sensor readings, and database writes).
\end{itemize}

\subsubsection{ESP32 Firmware Standards}
\begin{itemize}[leftmargin=1.2cm]
    \item Clear separation of concerns: sensor reading, networking, 
    and actuation logic implemented as separate functions 
    \cite{hassebo2024esp32}.
    \item Debouncing and filtering used for noisy sensors (e.g., 
    rain sensor) to avoid false triggers \cite{noisyiot2023}.
    \item Safe defaults: irrigation is turned \textbf{OFF} when 
    network or sensor failures occur to avoid uncontrolled watering 
    \cite{hassebo2024esp32}.
\end{itemize}

\subsubsection{Backend Standards (Node.js)}
\begin{itemize}[leftmargin=1.2cm]
    \item REST endpoints follow consistent URI patterns and HTTP 
    status codes \cite{thilakarathne2023fields}.
    \item Middleware-based validation and authentication (JWT) is 
    enforced for protected routes \cite{jwtauth2017}.
    \item Logging includes request IDs and timestamps for debugging 
    and audit.
\end{itemize}

\subsubsection{Mobile Application Standards}
\begin{itemize}[leftmargin=1.2cm]
    \item UI components follow a consistent design system for 
    readability and ease of use \cite{mobileui2022}.
    \item Data is fetched asynchronously and cached for better user 
    experience.
    \item Input forms (thresholds, schedules) validate values before 
    sending to backend.
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Embedded/Firmware:} Arduino IDE with ESP32 board 
    support packages \cite{raj2021survey}.
    \item \textbf{Backend:} Node.js runtime with Express.js 
    framework \cite{thilakarathne2023fields}.
    \item \textbf{Database:} MySQL (MariaDB) for all data storage 
    and retrieval operations \cite{uotila2020db}.
    \item \textbf{Mobile:} Android Studio (Android) or Flutter SDK 
    (cross-platform) \cite{khawas2018firebase}.
    \item \textbf{AI/ML:} Python environment with TensorFlow / 
    Scikit-learn \cite{ali2025soilml}.
    \item \textbf{Version Control:} Git for collaborative 
    development and change tracking \cite{sommerville2016software}.
\end{itemize}

\subsubsection{Testing Setup}
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Firmware testing:} Serial monitor logging and 
    controlled sensor value simulation \cite{hassebo2024esp32}.
    \item \textbf{Backend testing:} API testing using Postman and 
    unit checks for validation logic.
    \item \textbf{Mobile testing:} Emulators and physical device 
    testing for UI and notifications \cite{khawas2018firebase}.
    \item \textbf{End-to-end testing:} Real sensor readings sent to 
    backend, verified on app dashboard and alert system 
    \cite{emon2025iot}.
\end{itemize}
...
for local offline automation and zero-latency safety decisions \cite{goap2018iot}.
...
percentage values (0--100\%) before transmission to the backend, standardizing the data format \cite{saha2024automated}.
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Noisy Sensor Readings:} Soil moisture and rain 
    sensors produced unstable values in early tests. This was 
    resolved by applying filtering, threshold smoothing, and 
    debouncing logic \cite{noisyiot2023}.
    \item \textbf{Intermittent Wi-Fi Connectivity:} Network 
    dropouts caused missed telemetry uploads. Automatic reconnect 
    and retry mechanisms were introduced in firmware 
    \cite{farhan2019iotplatform}.
    \item \textbf{Consistent Data Synchronization:} Ensuring the 
    dashboard always reflects the latest reading required consistent 
    timestamp handling and backend validation 
    \cite{thilakarathne2023fields}.
\end{itemize}
