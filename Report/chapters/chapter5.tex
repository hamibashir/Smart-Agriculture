...
\subsubsection{Hardware Prototype Evolution and Waterproofing}
During the initial development and testing phase, the hardware components were mounted on a basic cardboard prototype to verify electrical connections and system logic. However, since the system is designed for field deployment where it will be exposed to water and harsh environmental conditions, strict waterproofing was mandatory. 

To achieve this, the entire ESP32 circuitry and power modules were enclosed in a durable plastic casing. Precise holes were drilled only for the essential sensor wires, and high-grade silicon sealant was generously applied around these openings to ensure no water could enter or damage the internal electronics. This resulted in a robust, waterproof, and ready-to-use final hardware prototype suitable for actual agricultural environments.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter5/testing2.jpg}
    \caption{Initial Hardware Prototype and Lab Testing Setup}
    \label{fig:prototype-initial}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter5/testing1.png}
    \caption{Final Waterproofed Ready-to-Use Hardware Prototype}
    \label{fig:prototype-final}
\end{figure}
% ----------------------------------------------------------
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter5/backend_health_api.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter5/ai_health_api.png}
...
\subsubsection{Deployment and Installation}
To simplify the deployment process for end-users, the complete source code and the latest pre-built APK for the mobile application have been published on our official GitHub repository \cite{smart_agri_github}. 

Users are not required to manually compile the application. They can simply navigate to the repository, download the latest compiled APK file, and install it directly on their Android smartphones (ensuring that ``Install from unknown sources'' is enabled in their device settings). Once installed, the user can immediately register, sign in, and connect to the live backend to start utilizing the Smart Agriculture system without any complex setup procedures.
