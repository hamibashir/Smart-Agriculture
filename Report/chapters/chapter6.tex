...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter6/serial_monitor_logs.png}
    \caption{ESP32 Serial Monitor Logs}
    \label{fig:serial_monitor_logs}
\end{figure}

Figure \ref{fig:serial_monitor_logs} displays the active serial monitor output directly from the ESP32 microcontroller during the testing phase. The logs confirm successful network connectivity, including Wi-Fi attachment and MQTT broker connection. Furthermore, the monitor actively prints real-time sensor readings (such as temperature, humidity, and soil moisture levels) along with physical actuator triggers. This validates that the local hardware firmware is successfully reading environmental data and dispatching telemetry payloads to the backend server.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter6/api_output_backend.png}
    \caption{Backend API Health Endpoint Response}
    \label{fig:api_output_backend}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter6/api_output_ai.png}
    \caption{AI Service API Health Endpoint Response}
    \label{fig:api_output_ai}
\end{figure}

To ensure seamless communication between the client applications and the server, API output tests were conducted against the live production endpoints. As shown in Figure \ref{fig:api_output_backend}, the backend API (\texttt{https://api.hamzabashir.online/health}) correctly processed the request and returned a JSON response confirming its active status and production environment. Similarly, as depicted in Figure \ref{fig:api_output_ai}, the AI service endpoint was successfully queried. It returned a structured JSON payload detailing its supported crops (e.g., chickpea, cotton, wheat), the required input features, and a verified model accuracy of $87.86\%$. These successful interactions verify the robustness of the RESTful architecture, confirming that routing and AI model inference are functioning securely and efficiently.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/chapter6/sensor_reading_validation.png}
    \caption{MySQL Database Sensor Reading Validation}
    \label{fig:sensor_reading_validation}
\end{figure}

Figure \ref{fig:sensor_reading_validation} provides a direct view into the connected MySQL database structure. While the database houses several management tables, this specific screenshot validates the telemetry data pipeline by showcasing the live sensor readings table. It confirms that the environmental data transmitted by the ESP32 field unit over the internet is successfully received, processed, and permanently stored in the backend database. This persistent data storage acts as the critical foundation for historical analytics and the AI crop recommendation engine.
