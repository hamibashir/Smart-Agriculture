...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth, height=6cm]{figures/chapter3/System_Architecture.png}
...
transmission, processing, and user interaction.
...
% Figure: ESP32 Circuit Schematic
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/chapter3/esp32_circuit_schematic.png}
    \caption{ESP32 Component Wiring Schematic (Fritzing)}
    \label{fig:esp32_circuit_schematic}
\end{figure}

The detailed wiring diagram presented in Figure \ref{fig:esp32_circuit_schematic} was developed using Fritzing to map out the precise electrical connections between the ESP32 microcontroller and the peripheral sensors. It illustrates the digital and analog pin assignments for the DHT11 sensor, soil moisture probe, rain sensor, LDR, and the relay module connected to the water pump.

% Figure: Hardware Layout
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/chapter3/hardware_wiring_layout1.jpg}
    \caption{Hardware Components and Sensor Modules Overview}
    \label{fig:hardware_components}
\end{figure}

Figure \ref{fig:hardware_components} presents a high-level visual overview of the individual hardware components utilized in the system before their final enclosure. The image highlights the core ESP32 microcontroller, the FC-28 soil moisture sensor, and the active relay module during the initial lab testing phase. This component-level view confirms the physical procurement and basic operational status of the essential IoT nodes required for the smart agriculture system.
...
Capacitive sensor (YL-69) that measures
volumetric water content in soil. Provides analog output (0--2560)
...
\textbf{Rain Sensor} &
Digital rain detection module (FC-37) with adjustable
...
\textbf{Water Pump} &
12V DC submersible pump for irrigation
...
regulator). Separate 12V supply for pump with protection
...
To generate intelligent crop recommendations, the system utilizes a Machine Learning pipeline driven by a \textbf{Random Forest Classifier}.


% Figure: AI Dataset Sample
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/chapter4/ai_dataset_sample.png}
    \caption{Training Dataset Sample for Crop Recommendation}
    \label{fig:ai_dataset}
\end{figure}

Figure \ref{fig:ai_dataset} presents a representative sample of the localized agricultural dataset used to train the Random Forest classification model. The tabular structure clearly illustrates the multi-dimensional input feature space, comprising continuous environmental variables (soil moisture, temperature, humidity) and categorical agronomical constraints (soil type and season). Each structured record directly maps these five independent variables to an optimal target variable (\texttt{recommended\_crop}), providing the foundational ground truth necessary for the machine learning algorithm to learn complex, non-linear environmental relationships.
 The model maps real-time environmental context to optimal crop varieties.
...
The Random Forest model outputs a confidence score alongside its primary crop recommendation. This score is mathematically derived using the algorithm's ensemble soft-voting probability mechanism, directly mirroring the \texttt{predict\_proba} function implemented in the AI backend. Specifically, for an input vector $x$, each of the 150 decision trees ($N_{trees} = 150$) calculates a class probability $p_t(c|x)$, representing the fraction of samples of class $c$ in the resulting leaf node. The final class probability $P(c|x)$ is the mean of these probabilities across the entire forest:
\begin{equation}
P(c|x) = \frac{1}{N_{trees}} \sum_{t=1}^{N_{trees}} p_t(c|x)
\end{equation}
The confidence score ($C$) is then extracted as the maximum mean probability among all supported crop classes, converted to a percentage:
\begin{equation}
C = \max_c P(c|x) \times 100\%
\end{equation}
...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    the relay to turn the pump/valve ON.
...
As illustrated in Figure \ref{fig:component-diagram}, the UML Component Diagram formally defines the structural relationships between the system's core modules. It highlights the Application, Backend, AI, and Device layers, explicitly showing how the Node.js API acts as the central hub. The defined interfaces, such as the REST API (HTTPS) for the mobile app and MQTT/Wi-Fi for telemetry, ensure that each component remains modular, scalable, and loosely coupled.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/chapter3/component diagram.png}
    \caption{UML Component Diagram of the System}
    \label{fig:component-diagram}
\end{figure}

% ----------------------------------------------------------
% 3.9 UML Activity Diagram
% ----------------------------------------------------------
\subsection{UML Activity Diagram}
\label{sec:uml-activity}

Figure \ref{fig:activity-diagram} presents the comprehensive UML Activity Diagram, mapping out the dynamic execution flow of the system. It demonstrates the initial authentication, branching into parallel tracks for background system operations (sensor reading, AI analysis) and active user interactions (monitoring, manual control). The synchronization of these parallel tracks ensures that the mobile dashboard reflects real-time telemetry while simultaneously triggering appropriate Firebase push notifications for critical alerts.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/chapter3/activity diagram.png}
    \caption{UML Activity Diagram of the System Workflow}
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Arduino IDE:} Embedded development 
    ...
    \item \textbf{Python ML Model:} Random Forest 
